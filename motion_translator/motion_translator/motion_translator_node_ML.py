...
class MotionTranslator(Node):
    ...
    def _target_for_command(self, cmd):
        if cmd == "Stop":
            ...
            # If stopped, start moving forward at linear_step speed
            return self.current_linear if self.current_linear > 0 else self.linear_step, 0.0
        elif cmd == "Continue moving backward":
            # If stopped or moving forward, start moving backward at -linear_step speed
            return self.current_linear if self.current_linear < 0 else -self.linear_step, 0.0
        elif cmd == "Turn left":
            ...
            # Increase magnitude, keep direction
            ...
            # Decrease magnitude, keep direction
            if self.current_linear > 0:
                return max(self.current_linear - self.linear_step, 0.0), 0.0
            elif self.current_linear < 0:
                return min(self.current_linear + self.linear_step, 0.0), 0.0
            else:
                return 0.0, 0.0
        else:
